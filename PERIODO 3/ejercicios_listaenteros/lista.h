...
int ListaEnteros::obtener(int indice){
	...
	if (this->estaVacia()) throw "No se ppuede recuperar elementos de una lista vacia";
	...
	void ListaEnteros::insertar(int indice, int item){
		...
		for(int i = cuenta - 1; i >= indice; i--){
			this->items [i+1] = this->items[1];
		}
		
		this->items[indice] = items;
		this->cuenta++;
	}
	...
	bool ListaEnteros::contine(int item){
		for(int i = 0; i < this->cuenta; i++){
